...
`VictoryLegend` uses the standard `externalEventMutations` prop. [Read about it in detail](/docs/api/victory-common-theme-props#externaleventmutations)
...
`VictoryLegend` uses the standard `labelComponent` prop. [Read about it here](/docs/api/victory-datatable-props#datacomponent)
