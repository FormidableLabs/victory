...
*note:* When providing tooltips for `VictoryLine` or `VictoryArea`, it is necessary to use [`VictoryVoronoiContainer`](/docs/api/victory-voronoi-container), as these components only render a single element for the entire dataset.

See the [tooltips guide](/docs/guides/tooltips) for more expanded examples of usage.
...
>
  <VictoryBar
    ...
    labelComponent={<VictoryTooltip dy={0} centerOffset={{ x: 25, y: -25 }} />}
  />
</VictoryChart>
```
...
>
  <VictoryBar
    ...
    labelComponent={
      <VictoryTooltip
        style={{ fill: "tomato", stroke: "black", strokeWidth: 2 }}
      />
    }
  />
</VictoryChart>
