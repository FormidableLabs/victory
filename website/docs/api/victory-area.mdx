...
`VictoryArea` overrides the standard [`dataComponent`](/docs/api/common-props#datacomponent) prop and supplies the following props to its `dataComponent`:
...
`VictoryArea` uses the standard `eventKey` prop. [Read about the `eventKey` prop in more detail here](/docs/api/common-props#eventkey)
...
See the [Events Guide](/open-source/victory/guides/events) for more information on defining events.
...
:::note
`VictoryArea` uses [`VictoryClipContainer`](/open-source/victory/api/victory-clip-container) as its default `groupComponent` `VictoryClipContainer` renders a `<g>` tag with a `clipPath` `def`. This allows continuous data components to transition smoothly when new data points enter and exit. 
:::
...
:::note
To enable tooltips on `VictoryArea`, it is necessary to use [`VictoryVoronoiContainer`](docs/victory-voronoi-container)
:::
