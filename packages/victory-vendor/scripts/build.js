...
const main = async () => {
  ...
  const pkgs = (await fs.readdir(path.resolve(__dirname, "../node_modules/")))
  ...
  await execa("yarn" /*yarn*/, [
    "babel",
    "--config-file", path.resolve(__dirname, "../.babelrc.js"),
    "-d", path.resolve(__dirname, "../lib-vendor"),
  ...
  for (let pkgName of pkgs) {
    ...
    await Promise.all([
      fs.writeFile(
        path.join(EsmBasePath, `${pkgName}.js`),
        getEsmIndex(pkg)
      ),
      fs.writeFile(
        path.join(CjsBasePath, `${pkgName}.js`),
        getCjsIndex(pkg)
      ),
      fs.copyFile(
