...
const Helpers = {
  ...
  onMouseDown(evt, targetProps) { // eslint-disable-line max-statements, complexity
    evt.preventDefault();
    const {
    ...
    if (activeHandles) {
      ...
      return allowDraw ? [{
        target: "parent",
        mutation: () => ({
          isSelecting: allowResize, domainBox, fullDomainBox, parentSVG,
  ...
  onMouseUp(evt, targetProps) {
    const {
      x1, y1, x2, y2, onBrushDomainChangeEnd, onBrushCleared, domain, allowResize, defaultBrushArea
    } = targetProps;
