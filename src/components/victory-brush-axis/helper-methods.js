...
export default {
  ...
  getStyles(props, styleObject) {
    ...
    return {
      ...
      brushArea: defaults({}, style.brushArea, styleObject),
      brush: defaults({}, style.brush, styleObject.brush),
      brushHandles: defaults({}, style.brushHandles, styleObject.brushHandles),
  ...
  getBrushAreaProps(props, calculatedValues, globalTransform) {
    const { style, padding, isVertical } = calculatedValues;
    const { width, height } = props;
    const brushWidth = style.brushArea.width !== undefined ?
      style.brushArea.width : props.brushWidth;
    return {
      x: isVertical ? globalTransform.x - brushWidth / 2 : padding.left + globalTransform.x,
      y: isVertical ? padding.top + globalTransform.y : globalTransform.y - brushWidth / 2,
      width: isVertical ? brushWidth : width - padding.right - padding.left,
      height: isVertical ? height - padding.bottom - padding.top : brushWidth
    };
  },
  ...
  getBaseProps(props, fallbackProps) {
    ...
    const brushAreaProps = this.getBrushAreaProps(props, calculatedValues, globalTransform);
