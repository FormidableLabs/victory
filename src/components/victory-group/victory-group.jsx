...
export default class VictoryGroup extends React.Component {
  ...
  getX0(datum, index, calculatedProps) {
    const {categories, datasets, style} = calculatedProps;
    const x = datum.x;
    ...
    if (datum.category !== undefined) {
      ...
      return (bandCenter + (centerOffset * totalWidth)) - x;
    }
    return centerOffset * totalWidth;
  }

  addLayoutData(datasets, index, calculatedProps) {
    return datasets[index].map((datum) => {
      return assign(datum, {
        xOffset: this.getX0(datum, index, calculatedProps),
      })
    });
  }
  ...
  getChildStyle(child, index, calculatedProps) {
    ...
    const defaultFill = this.getColor(this.props, index);
