...
export default class VictoryChart extends React.Component {
  ...
  getCalculatedProps(props, childComponents) {
    const flipped = this.props.flipped || this.props.children.every(
      (component) => component.props.horizontal
    );
    const datasets = this.props.children.map((child) => {
      return child.getData(child.props) : Data.getData(child.props);
    });
    const domain = {
