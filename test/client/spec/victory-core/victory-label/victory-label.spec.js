...
import { VictoryLabel, Text, TSpan } from "victory-core";

describe("components/victory-label", () => {
  it("has expected content with shallow render", () => {
    const wrapper = shallow(<VictoryLabel text={"such text, wow"} />);
  ...
  it("renders an array of text as seperate tspans", () => {
    const wrapper = shallow(<VictoryLabel text={["one", "two", "three"]} />);
    const output = wrapper.find(TSpan);
    expect(output.length).to.equal(3);
  });

  it("renders splits newlines into tspans", () => {
    const wrapper = shallow(<VictoryLabel text={"one\ntwo\nthree"} />);
    const output = wrapper.find(TSpan);
    expect(output.length).to.equal(3);
  });
